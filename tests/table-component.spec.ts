...
test.describe('Table component', async () => {
  ...
  test('should display actions of remove, move up/down of rows with radio selection', async ({page}) => {
    ...
    await expect(PWUtils.getTableCell(table, 1, 1).locator('input')).toHaveValue('2c');
    await expect(PWUtils.getTableCell(table, 1, 2).locator('input')).toHaveValue('2a');
    await expect(PWUtils.getTableCell(table, 1, 3).locator('input')).toHaveValue('2b');
    ...
    await expect(PWUtils.getTableCell(table, 2, 3).locator('input')).toHaveValue('1b');
    ...
    await expect(PWUtils.getTableCell(table, 3, 1).locator('input')).toHaveValue('1c');
    await expect(PWUtils.getTableCell(table, 3, 2).locator('input')).toHaveValue('1a');
    await expect(PWUtils.getTableCell(table, 3, 3).locator('input')).toHaveValue('1b');
    await expect(PWUtils.getTableCell(table, 3, 4).locator('input')).toHaveValue('1');

    await expect(PWUtils.getTableCell(table, 2, 1).locator('input')).toHaveValue('3c');
    await expect(PWUtils.getTableCell(table, 2, 2).locator('input')).toHaveValue('3a');
    await expect(PWUtils.getTableCell(table, 2, 3).locator('input')).toHaveValue('3b');
    ...
    await expect(PWUtils.getTableCell(table, 3, 1).locator('input')).toHaveValue('4c');
    await expect(PWUtils.getTableCell(table, 3, 2).locator('input')).toHaveValue('4a');
    await expect(PWUtils.getTableCell(table, 3, 3).locator('input')).toHaveValue('4b');
    ...
    // Robust assertion: allow missing system key in valueCoding
    const expectedOptions = [
      { valueCoding: {system: '2c', display: '2a', code: '2b'}, initialSelected: true },
      { valueCoding: {system: '1c', display: '1a', code: '1b'} },
      { valueCoding: {system: '4c', display: '4a', code: '4b'} },
      { valueCoding: {display: 'xx'} }
    ];

    expect(q.item[0].answerOption).toEqual([
      {
        valueCoding: {system: '2c', display: '2a', code: '2b'},
        extension: [{
  ...
  test('should display actions of remove, move up/down of rows with checkbox selections', async ({page}) => {
    ...
    expect(q.item[0].answerOption).toEqual([
      ...
      {
        valueCoding: {system: '3c', display: '3a', code: '3b'},
        extension: [{
          url: "http://hl7.org/fhir/StructureDefinition/itemWeight",
          valueDecimal: 3
        }]
